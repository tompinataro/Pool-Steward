...
export default function AboutScreen({ navigation }: Props) {
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>About Pool Steward</Text>
      <Text style={styles.sub}>App v{version}</Text>
      {loading ? <ActivityIndicator /> : <Text style={styles.body}>{result ?? 'No result yet'}</Text>}
      <View style={{ height: 8 }} />
      <Pressable onPress={() => Linking.openURL('https://tompinataro.github.io/Pool-Steward/support').catch(() => {})} accessibilityRole="link" accessibilityLabel="Open Support">
        <Text style={styles.link}>Support</Text>
      </Pressable>
      <Pressable onPress={() => Linking.openURL('https://tompinataro.github.io/Pool-Steward/privacy').catch(() => {})} accessibilityRole="link" accessibilityLabel="Open Privacy Policy">
